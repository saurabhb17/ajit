...
serial_isr_label:
	...
	mov %psr, %l0
        ...
        ! need to save the globals somewhere.
        std %g0, [%sp +  0]
        std %g2, [%sp +  8]
        std %g4, [%sp + 16]
        std %g6, [%sp + 24]
	...
	/*To return to the instruction after the trapped instruction
	(for example, after emulating an instruction) use the sequence:*/
	/*	
	jmpl %r18,%r0 ! old nPC
...
HW_trap_0x1c: 
	/*disable irc happens in __serial_isr(), NOT directly here. Is it OK?
	Yes, because ET=0 as soon as PC comes to trap_vector_table*/
	ba serial_isr_label; nop; nop; nop; 
